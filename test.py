...
for i in range(len(test_source_file_list)):
    ...
    test_source_signal = cf.create_signal(test_source_signal, config['upsample_size'])
    ...
    wav.write_wav(result[:len(result) - test_mod], "{}/{}".format(result_path, file_name), test_source_sample_rate * config['upsample_size'])
